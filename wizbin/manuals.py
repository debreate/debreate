...
class Page(WizardPage):
	...
	def __init__(self, parent):
		...
		self.SetTitle(GT("Manual Pages"))

		self.Tabs = MultiTemplate(self, ManPage)

		self.Tabs.RenameButton(btnid.ADD, GT("Add Manual"))
		self.Tabs.RenameButton(btnid.RENAME, GT("Rename Manual"))
		self.Tabs.AddTabButton(GT("Switch Mode"), "mode", btnid.MODE, self.OnChangeMode)
	...
	def AddManpage(self, name="manual", easyMode=True):
	...
	def OnCloseTab(self, event=None):
		Logger.Debug(__name__, "Closing tab")
	...
	def SetTabName(self, index=-1, rename=False):
		getname = TextEntryDialog(GetMainWindow(), GT("Name for new manpage"))
		new_title = None

		if not rename:
			easy_mode = CheckBox(getname, label="Easy mode", defaultValue=True)
		...
		while not valid_name:
			...
			ShowErrorDialog(GT("Manpage name cannot contain whitespace"), warn=True)
