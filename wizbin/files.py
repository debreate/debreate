...
class Page(WizardPage):
    ...
    def __init__(self, parent):
        ...
        self.chk_preserve_top = CheckBoxCFG(pnl_treeopts, label=GT("Preserve top-level directories"),
    ...
    def ExportBuild(self, target):
        if not os.path.isdir(target):
            return (dbrerrno.ENOENT, GT("Target directory does not exist: {}").format(target))

        if self.lst_files.MissingFiles():
            return (dbrerrno.ENOENT, GT("Missing files in file list"))
        ...
        for R in range(file_count):
            ...
            stage_dir = "{}/{}".format(target, stage_dir).replace("//", "/")
            staged_file = "{}/{}".format(stage_dir, filename).replace("//", "/")

            filename = "{}/{}".format(source_dir, filename).replace("//", "/")
            Logger.Debug(__name__,
                    GT("Coppying file: {} -> {}").format(filename, stage_dir))
            ...
            if executable:
                os.chmod(staged_file, 0o0755)

            if not os.path.isfile(staged_file):
                return(dbrerrno.ENOENT, GT("File was not copied to stage: {}").format(staged_file))
    ...
    def Get(self, getModule=False):
        ...
        if not item_count:
            ...
            for X in range(item_count):
                ...
                filename = "{}/{}".format(row_data[1], row_data[0])
                target = row_data[2]
    ...
    def Set(self, data):
        ...
        if int(files_data[0]):
            ...
            while current_file > 1:
                ...
                absolute_filename = files_data[current_file].split(" -> ")[0]
                ...
                target_dir = files_data[current_file].split(" -> ")[2]
