...
class AboutDialog(wx.Dialog):
  ...
  def __init__(self, parent, size=(600,558)):
    ...
    if OS_name:
      ...
      if OS_version:
        os_info.SetLabel("{} {}".format(os_info.LabelText, OS_version))

      if OS_codename:
        os_info.SetLabel("{} {}".format(os_info.LabelText, OS_codename))
      ...
      if OS_upstream_name:
        ...
        if OS_upstream_version:
          os_upstream_info.SetLabel("{} {}".format(os_upstream_info.LabelText, OS_upstream_version))

        if OS_upstream_codename:
          os_upstream_info.SetLabel("{} {}".format(os_upstream_info.LabelText, OS_upstream_codename))
  ...
  def SetChangelog(self):
    ...
    if INSTALLED:
      ...
      CHANGELOG = "{}/share/doc/debreate/changelog.gz".format(PREFIX)

    else:
      CHANGELOG = "{}/docs/changelog".format(PREFIX)
    ...
    if os.path.isfile(CHANGELOG):
      ...
      if changelog_mimetype == "application/gzip":
        ...
        if CMD_gzip:
          ...
          res = subprocess.run([CMD_gzip, "-fd", os.path.basename(CHANGELOG)], capture_output=True)
          gzip_output = (res.returncode, res.stdout.decode("utf-8"))

          Logger.Debug(__name__,
              GT("gzip decompress; Code: {}, Output: {}").format(gzip_output[0], gzip_output[1]))

          os.chdir(prev_dir)

        changelog_file = os.path.basename(CHANGELOG).split(".")[0]
        changelog_file = "{}/{}".format(temp_dir, changelog_file)
      ...
      elif changelog_mimetype == "text/plain":
  ...
  def SetLicense(self):
    ...
    #   to reflect installed path.
    if INSTALLED:
      license_path = "{}/share/doc/debreate/copyright".format(PREFIX)

    else:
      license_path = "{}/docs/LICENSE.txt".format(PREFIX)
