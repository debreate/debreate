...
class ManPage(wx.Panel):
  def __init__(self, parent, name="manual", easy_mode=True):
  ...
  def RemoveDocumentSection(self, index):
    if index == None:
      Logger.Error(__name__, "Cannot remove desired index")

      return False

    Logger.Debug(__name__, "Removing manpage section at index {}".format(index))
  ...
  def SetModeEasy(self):
    ...
    menu_add = PanelMenu(menubar, label=GT("Add"))
    menu_add.Append(manid.SINGLE, GT("Single line section"))
    menu_add.Append(manid.MULTILINE, GT("Multi-line section"))
    ...
    txt_banners = wx.StaticText(self.pnl_bottom, label=GT("Banners"))
    ...
    self.AddDocumentSection(GT("Name"))
    self.AddDocumentSection(GT("Synopsis"), style=DEFAULT_MANSECT_STYLE|manid.MULTILINE)
    self.AddDocumentSection(GT("Description"), style=DEFAULT_MANSECT_STYLE|manid.MULTILINE)
    self.AddDocumentSection(GT("Options"), style=DEFAULT_MANSECT_STYLE|manid.MULTILINE)
    self.AddDocumentSection(GT("Examples"), style=DEFAULT_MANSECT_STYLE|manid.MULTILINE)
    self.AddDocumentSection(GT("See also"))
